...
type DesktopShellProps = {
  /**
   * Header account control: an avatar button that opens a dropdown with the
   * user details, debug info, and the single Switch user action (v0.2.1).
   */
  userMenu: ReactNode;
};
 ...
 * this frame; only the header user menu and the auth guard differ.
 *
 * The header is a single compressed row (v0.2.1): app title on the left, the
 * avatar `userMenu` on the right. Identity details, user id, the version badge,
 * and Switch user moved into the dropdown so the bar stays one line.
 *
 ...
 */
function DesktopShell({ userMenu }: DesktopShellProps) {
  return (
      ...
      <div className="mx-auto flex min-h-screen max-w-7xl flex-col bg-neutral-50 shadow-sm">
        <header className="sticky top-0 z-20 border-b border-neutral-200 bg-neutral-50/95 backdrop-blur">
          <div className="flex h-14 items-center justify-between gap-3 px-4">
            <h1 className="shrink-0 text-lg font-bold tracking-normal">
              Local Feed
            </h1>
  ...
  return (
    <DesktopShell
      userMenu={
        <UserMenu
          avatar={
            <div className="flex size-8 shrink-0 items-center justify-center rounded-full bg-neutral-200 text-sm font-bold text-neutral-600 ring-1 ring-neutral-200">
              {avatarInitial}
            </div>
          }
          primaryLabel={`@${activeApiUser.handle}`}
          secondaryLabel="API mode"
          meta={[
            { label: 'User id', value: activeApiUser.id },
            { label: 'Mode', value: 'API' },
...
function MockModeAppShell() {
  ...
  const avatarNode = activeUser.avatar ? (
    <img
  ...
  return (
    <DesktopShell
      userMenu={
        <UserMenu
          avatar={avatarNode}
          primaryLabel={activeUser.display_name}
          secondaryLabel={`@${activeUser.handle}`}
          meta={[
