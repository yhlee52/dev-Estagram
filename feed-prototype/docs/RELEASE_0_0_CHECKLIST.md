...
- [ ] `scripts/check_account_lifecycle.py`가 통과합니다(v0.6.4, 탈퇴/비활성 + post 보존).
- [ ] (선택) `scripts/reset_password.py`로 임의 seed user의 password를 재설정하고
  이전 password 실패 / 새 password 로그인 성공을 확인합니다(v0.6.3).
- [ ] (선택) `scripts/reactivate_user.py`로 비활성 계정을 재활성화하면 로그인/노출이
  복귀합니다(v0.6.4).
...
- [ ] Me 탭 Danger zone에서 계정 비활성화 → 로그아웃되어 로그인 화면 복귀, 이후 로그인
  차단(403), Accounts 목록에서 제외, 작성한 post는 보존됩니다(v0.6.4). 운영자
  `reactivate_user`로 복구됩니다.
- [ ] Home Feed가 표시됩니다.
